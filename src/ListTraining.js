...
import VenueCategory from './components/VenueCategory/VenueCategory';
import TrainingCategory from './components/TrainingCategory/TrainingCategory';
import SpecificVenue from './components/SpecificVenue/SpecificVenue';
import SpecificTraining from './components/SpecificTraining/SpecificTraining';
import MobSpecTrainingDetailsForm from './pages/MobSpecTrainingDetailsForm/MobSpecTrainingDetailsForm';
import Amenities from './components/Amenities/Amenities';
import ModeofTraining from './components/ModeofTraining/ModeofTraining';
import Availability from './components/availability1/availability';
import VenueAvailability from './components/Availability/VenueAvailability';
...
import IndividualSuccessModel from './components/IndividualSuccessModel/IndividualSuccessModel';
...
const errorStatus=[
			...
			{key:'trainingcategory',message:"Please Select the training category",status:false},
			{key:'specifictraining',message:"Please Select the specific training and details",status:false},
			{key:'modeoftraining',message:"Please Select the modeoftraining and details",status:false},
...
export default class ListTraining extends React.Component {

	constructor(props) {
		super(props);
		console.log('mainprops',props);
		this.state = {venue_id:null,purposeData:null,
			successmodal:false,
			fileList:null,
			imageData:null,
			tagsDetails:null,availobj:null,priceobj:null,activeid:1,
			"venue_cat_id":null,"veneu_spec_id":null,categoryName:'',
			commonArray:null,amentiesactiveobj:null,amenitiesArray:null,
...
// alert(JSON.stringify(data))
	...
	render() {
		console.log("training props",this.props);
		return (
       ...
       {window.innerWidth > 767 &&
        ...
        <VenueHeader showlisthome={this.showlisthome} showBreadCrums={this.state.breadCrums}/>
			...
			{this.state.activeid==1&&
				<TrainingCategory categoryId={this.state.venue_cat_id} trainingCategoryCallback={(data,name)=>this.changeDynamic(data,'id',name,'categoryName',2)} changeActive={(data)=>{this.setState({activeid:data})}} checkValidationError={this.checkValidationError}/>
			}

			{this.state.activeid==2&&
				<SpecificTraining commonArray={this.state.commonArray} sendCommonDetails={(data)=>this.changeDynamic(data,'commonArray',null,null,3)} specificId={this.state.veneu_spec_id} categoryname={this.state.categoryName} venuespecCallback={(data,mob)=>this.changeDynamic(data,'veneu_spec_id',null,null,mob?2.5:3)} venueCatId={this.state.venue_cat_id} changeActive={(data)=>{this.setState({activeid:data})}} checkValidationError={this.checkValidationError}/>		
			}

			{this.state.activeid==2.5&&
				<MobSpecTrainingDetailsForm checkValidationError={this.checkValidationError} commonArray={this.state.commonArray}  venuespecCallback={(data,mob)=>this.changeDynamic(data,'veneu_spec_id',null,null)} sendCommonDetails={(data)=>this.changeDynamic(data,'commonArray',null,null)} categoryname={this.state.categoryName} venueSpecDetails={this.state.veneu_spec_id} changeActive={(data)=>{this.setState({activeid:data})}} />
			}


			{
				this.state.activeid==3 &&
				  <ModeofTraining  categoryId={this.state.venue_cat_id} trainingCategoryCallback={(data,name)=>this.changeDynamic(data,'id',name,'categoryName',2)} changeActive={(data)=>{this.setState({activeid:data})}} checkValidationError={this.checkValidationError} />
			}

			{this.state.activeid==4 &&
				<Availability  availobj={this.state.availobj} sendAvailabiltyProp={this.receiveAvailability} changeActive={(data)=>{this.setState({activeid:data})}} checkValidationError={this.checkValidationError}/>
			}
