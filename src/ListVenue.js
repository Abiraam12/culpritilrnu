 ...
 //        })
	...
	priceDataRecive=(data,calling)=>{
		...
		if(availobj.businessform.seatList.length>0){
			var seatDetails=data.seatList.length>0?data.seatList.map((obj)=>{return {seatName:obj.seatname,seatQuantity:obj.counter,seatPricing:obj.pricing,priceType:data.activePrice}}):[];
	...
	componentDidMount(){
		...
		if(this.props.location.state!=undefined){
			...
			if(seatData.length>0){
				...
				var seatorprice=[{seatname:'',counter:0,add:'true'}].concat(editingData.price.map((obj)=>{return {seatname:obj.seat_name,counter:obj.seat_qty,uniqueId:obj.seat_unique_id,pricing:obj.priceDetails.length>0?obj.priceDetails.map((seatobj)=>{
					var seatobj=seatobj;
		...
		if(editingData.trn_venue_type==2){
			paxListing=editingData.price.map((data)=>{
				var objData={paxName:data.venue_pax_name,paxMin:data.pax_min,paxMax:data.pax_max,details:data.venue_pax_desc,pricing:[],uniqueId:data.pax_unique_id}
