...
import CkEditorComp from '../../ckEditorComp/CKEditorComp';
...
// const calendar=require("../src/icon/calendar.svg");
...
const dayText = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
 ...
 class VenueAvailability extends React.Component {
	...
	constructor(props) {
        ...
        selectedDays:[],fromdate:'',todate:'',hourlydrop:{value:'id',name:'value',dropdown:hour_drop},'hourobj':hour_drop[0],businessform:{fromDate:new Date(),toDate:new Date(),fromTime:new Date(),toTime:new Date(),excludesDays:[],slot_type:1,min:null,max:null},SplitSlots:[],selectedSlots:[],editSLots:null,editvisible:null}
	}
...
receiveAvailability=(obj,state,hourobj1)=>{
...
submitDetails=()=>{
    console.log("Checking Form",this.props);
        var obj={activeobj:this.state.activeobj,hourobj:this.state.hourobj,businessform:this.state.businessform,SplitSlots:this.state.SplitSlots,selectedSlots:this.state.selectedSlots,slotType:this.state.slot_type,hourSlots:this.state.hourSlots,moreDetails:this.state.moreDetails};
...
receiveEditorData=(data)=>{
    ...
    End:new Date("2019-08-27T10:00:00.000Z"),
		...
		return (
			<div className="availablityform-maindiv">
				...
				{/* <Choosediv  content={HeaderContent} changeActive={(data)=>this.props.changeActive(data)} prev={3} next={5} activeArrow={this.state.activeArrow}/> */}
              <div className="main-div" style={{paddingLeft:"40px",marginTop:"40px"}}>
			  {btn_details.map((obj,i) => {
          return(
                <div style={{marginRight:"70px"}}>  <Specific disabled={this.state.businessform.slot_type=='2'} svg={true} width="60px" height="60px" filter="brightness(100)" undertext={obj.name} specificvenue={()=>this.receiveAvailability(obj)} text={obj.icon} active={this.state.activeobj&&this.state.activeobj.id==obj.id} activecolor={"#EA5B04"} Skew={true}/></div>
              )
          }
            )}
			  </div>
    ...
    <CkEditorComp moreDetails={this.state.moreDetails} sendEditorData={(data)=>this.receiveEditorData(data)}/>
           ...
           {this.state.activeobj.id==1&&this.state.businessform.slot_type==1&&this.state.showEditForm==false&&
            <div className="HourlyBlock">
            <div className="SlotDescription">
            <h3 className="Slots_h3">Slots</h3>
            ...
            <div className="avail_RadioGroup">
              ...
              <div className="RadioBtns Radio_toalign">
        <FormControlLabel
          value="1"
...
<div className="SlotsListBox">
 ...
 <SlotBox selected={true} textselectedColor={"white"} selctedBoxColor={"#eb5c00"}>{obj1.label}</SlotBox>
           ...
           {/*Hourly Block Ends*/}
           {(this.state.activeobj.id!=1 || this.state.businessform.slot_type==2 || this.state.businessform.slot_type==3 )&&this.state.showEditForm==false&&
           <div className="next-div AvailbilityBtnSubmit">
                <button onClick={()=>this.submitDetails()}  type="button" className="availablitysave-button"><span className="availabilitysave-span">Submit</span></button>
            </div>
                }
			</div>
			</div>
