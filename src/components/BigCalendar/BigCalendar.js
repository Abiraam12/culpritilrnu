...
var filterrecords=props.blockingDates.filter((obj)=>obj.selected_date==selectdDate&&obj.booking_slot_type!="");
if(filterrecords.length>0){
 ...
 function changeDateValue(date){
   changeDate(date);
   if(props.venutype==2 || props.venutype==3){
...
function UnblockDates(data){
  ...
  if(data.length>0){
    ...
    props.blockDates&&props.blockDates([data[0].trn_booking_slot_id],new Date(data[0].selected_date));
  }
}
function sendBlockingId(date,id){
...
return (         
<div  onDoubleClick={()=>isblockorbook?sendBlockingId(day,isblockorbook):null} className={`${isblockorbook&&isblockorbook==1?'Booked ':(isblockorbook==2?'Blocked ':'')}${props.type&&props.type!=1?generateClasses(startDate,endDate,dynamicDate,isblockorbook?isblockorbook:''):''}${noofslots?'badgecountdiv':''}`}>{(dateFormat(day,'yyyy-mm-dd')>=dateFormat(new Date(),'yyyy-mm-dd'))&&noofslots&&checkexcludeDays(props.excludeDays,day)&&noofslots!='0'?<div className="badgecount">{noofslots}</div>:checkexcludeDays(props.excludeDays,day)==false?<div className="holiday"></div>:''}{dayComponent}
      </div>
   )
}else{
  return dayComponent;
