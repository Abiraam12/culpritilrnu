		...
		return (
			...
			{!this.state.loading &&
				...
				{this.state.ListVenueType.map((item,i)=>
					<Specific  dummy={item.dontbind} width="100px" height="100px" filter="brightness(100)" undertext={item.venue_cat_name} specificvenue={()=>this.specificvenue(item)} text={img(item.venue_cat_icon)} active={(this.state.venuecategoryid)==item.venue_cat_id} activecolor={"#EA5B04"} Skew={true}/>
				)}
