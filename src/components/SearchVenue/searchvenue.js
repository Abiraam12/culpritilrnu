  ...
  fetch(Apilink.apiurl+'dropdownSearchpurpose', {
    ...
    onClick:() => {
     ...
     // this.setState({searchdata:responseJson});
      ...
      // this.loadOvalCount();
      this.props.receivesearch(responseJson,this.state.whatactiveobj);
        }
      }
      // this.setState({searchdata:responseJson});
    ...
    <Popupbox buttonColor={"white"} buttonTextColor={"black"} buttonText={"Soccer"}  height={50}/>
     ...
     <Nextarrow buttonColor={"#0f3eb0"}  width={76.8} fontsize={39} nextSearchFunc={this.arrowClick}/>
			...
			{this.state.doDropdown&&this.state.doDropdown.dropdown.concat(dummydata).map((item)=>{
				...
				return(
<Circle dummy={item.dummy}  width="110px" height="110px" circleChange={()=>this.circleChange(item,'doactiveobj')} text={item.venue_act_name} textcolor={'#ffffff'} active={this.state.doactiveobj&&this.state.doactiveobj.venue_act_id==item.venue_act_id} activecolor={"#EA5B04"}/>
...
<div className="venue-palces">
			{this.state.whatDropdown&&this.state.whatDropdown.dropdown.concat(dummydata).map((item)=>{
				...
				return(
<Circle dummy={item.dummy} width="110px" height="110px" circleChange={()=>this.circleChange(item,'whatactiveobj')} text={item.venue_purpose_name} textcolor={'#ffffff'} active={this.state.whatactiveobj&&this.state.whatactiveobj.venue_purpose_id==item.venue_purpose_id} activecolor={"#EA5B04"}/>
...
<div className="venue-palces">
			{this.state.whereDropdown&&this.state.whereDropdown.dropdown.concat(dummydata).map((item)=>{
				...
				return(
<Circle dummy={item.dummy} width="110px" height="110px" circleChange={()=>this.circleChange(item,'whereactiveobj')} text={item.venue_cat_name} textcolor={'#ffffff'} active={this.state.whereactiveobj&&this.state.whereactiveobj.venue_cat_id==item.venue_cat_id} activecolor={"#EA5B04"}/>
