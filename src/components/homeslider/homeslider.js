...
import Bradley from '../../img/RChHGbK-bradley-cooper-wallpapers.jpg'
...
},
{
 ...
 class Homeslider extends Component {
  constructor(props) {
    ...
    this.state = { activeIndex:0,dummyRefreshState:false,numberoflines:3,homeslider:[{
      id:1,
      name:"Arjun",
      photos:<img src={Bradley} alt="Logo" />
    },
  {
    id:2,
    name:"kishore",
    photos:<img src={Bradley} alt="Logo" />
  }],imageStatus:false,activeBookObj:null,BookVisible:false,LoginModelVisible:false,PhotoVisible:null };
...
loadCarouselImages=(data)=>{
  console.log("photo carosouel",data)
  ...
  render() {
    ...
    const slides = this.state.homeslider.map((item,key) => {
      ...
      return (
        ...
        <div className="">
        <span className="slider-header">{item.name}</span>
        </div>
        ...
        <div className="Homeslider PriceCard" >
          {/* <div>{item.trn_venue_type==2&&"Per Pax " ||item.trn_venue_type==3&&"Per Seat "}{item.price.length>0&&item.price[0].trn_venue_price_amt } - {item.price.length>0&&item.price[0].trn_venue_price_currency}
          </div> */}
    ...
    return (
      ...
      <button className="slider-button" onClick={()=>this.BookNow(this.state.homeslider[this.state.activeIndex])}>View Details</button>
      </div>
