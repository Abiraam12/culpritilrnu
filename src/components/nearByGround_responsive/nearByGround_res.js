...
class NearByGround_res extends React.Component {
 constructor(props) {
   ...
   this.state = {coords:{latitude:0,longitude:0},nearbyLocationData:[],categoryName:'',categoryDropdown:{id:'venue_cat_id','name':'venue_cat_name',dropdown:[{id:1,name:'Software Trainer'},{id:2,name:'Soft Skill Trainer'},{id:3,name:'Software Sr Trainer'},{id:4,name:"Softskill SME"}]},activeData:null};
 }
  ...
  render(){
    ...
    slidesPerView:'auto',
      ...
      pagination:{
		...
		<span className="nearby-text2">Your Favorites</span> 
		<Popupbox buttonText={this.state.dovalue?this.state.dovalue.venue_act_name:'Trainers'} sendPopupData={(data)=>this.receivedodrop(data,'dovalue')} dropdown={this.state.categoryDropdown} buttonColor={'#fff'} buttonTextColor={'#000'} popupColor={'white'} popupTextColor={'black'}/>
		</div>
...
<Homeslidercontent refresHeader={()=>this.props.refresHeader&&this.props.refresHeader()} activeCategory={this.state.activeData}/>
    }
