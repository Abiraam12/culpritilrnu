...
export default class Price extends React.Component {
	...
	constructor(props) {
		super(props);
		this.state={activeArrow:false,priceobj:{id:1,name:'USD'},top:0,height:0,left:0,activeobj:{'icon':<HourlySVG/>,'name':'Hourly','title':'Hour',id:1},amt:0,currencydropdown:[{id:1,name:'USD'},{id:2,name:'INR'},{id:3,name:'MYI'}],pricetypedetails:[{'icon':<HourlySVG/>,'name':'Hourly','title':'Hour',id:1,checked:true},{'icon':<DailySVG/>,'name':'Daily','title':'Day',id:2,checked:false},{'icon':<WeeklySVG/>,'name':'Weekly','title':'Week',id:3,checked:false},{'icon':<CalendarSVG/>,'name':'Monthly','title':'Month',id:4,checked:false}],activeCurrency:'USD',seatList:[{label:'',pricing:[]}],clearprice:null}
...
notification.error({
		...
		this.props.sendPriceData({seatList:this.state.seatList,'activeobj':this.state.activeobj,'priceobj':this.state.priceobj,'amt':this.state.amt,activeCurrency:this.state.activeCurrency,activePrice:this.state.pricetypedetails.filter((obj)=>obj.checked==true).map((obj2)=>obj2.id).join(),venueType:this.props.venueType});
			this.setState({activeArrow:true});
			this.props.checkValidationError('pricecard',true);
	}
}else{
	...
	<Choosediv  content={HeaderContent} changeActive={(data,action)=>this.props.changeActive(data,action)} prev={5} next={7} activeArrow={this.state.activeArrow}/>
		<div className="main-div">
			{this.state.pricetypedetails.map((obj,key)=>{
				return(
					<Specific type={this.props.venueType} disabled={this.props.venueType=='2'} svg={true} width="110px" height="110px" filter="brightness(100)" undertext={obj.title} specificvenue={()=>this.specificvenue(obj,obj.title,key)} active={(this.props.venueType==3 || this.props.venueType==1)?obj.checked==true:obj.id==this.state.activeobj.id} text={obj.icon}  activecolor={"#EA5B04"} Skew={false}/>
					)
			})}
