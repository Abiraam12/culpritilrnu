...
corporateNearYou=()=>{
 ...
 <Popupbox buttonText="Near You"  dropdown={{dropdown:[]}}  buttonColor={'transparent'} buttonTextColor={'black'} popupColor={'white'} popupTextColor={'black'}/>
      ...
      <img onLoad={()=>this.handleImageLoaded(item)}
           ...
           <div className="tagViewVenueType">{(item.trn_venue_type==2&& 'Per Pax' || item.trn_venue_type==3 &&'Per Seat ')} {item.price.length>0&&item.price[0].trn_venue_price_amt} - {item.price.length>0&&item.price[0].trn_venue_price_currency}</div>
         }
