...
import NearByPlayGround from "../nearByPlayGround/nearByPlayGround";
...
import homecss from "../../pages/IndividualPage/IndividualPage.css";
...
export default class Home extends React.Component {
  constructor(props) {
    ...
    this.state = {
      activeid: 1,
      visible: false,
      doDropdown: { id: "venue_act_id", name: "venue_act_name", dropdown: [] },
      whatDropdown: {
        id: "venue_spec_id",
  ...
  arrowClick = () => {
    fetch(Apilink.apiurl + "dropdownSearch", {
      ...
      body: JSON.stringify({
        trn_venue_name: this.state.wherevalue.trn_venue_name
      })
    })
  ...
  render() {
    console.log("responst",this.state.OvalButtonArray)
    return (
     ...
     {window.innerWidth > 767 &&
        ...
        <Header>
          ...
          {/* <Popupbox
            buttonText={
              this.state.countryvalue ? this.state.countryvalue.name : "Newyork"
            }
            dropdown={this.state.countryDropdown}
            buttonColor={"transparent"}
            buttonTextColor={"black"}
            sendPopupData={data => this.receivedodrop(data, "countryvalue")}
            popupColor={"white"}
        ...
        <Subheader bgcolor={"#a60202"}>
          <div className="home_droddownflex">
            ...
            <div className="home_dropdownflexItem">
              <Popupbox
                buttonText={
                  this.state.dovalue ? this.state.dovalue.venue_act_name : "DO"
                }
                sendPopupData={data => this.receivedodrop(data, "dovalue")}
                dropdown={this.state.doDropdown}
                buttonColor={"#fff"}
                buttonTextColor={"#000"}
                popupColor={"white"}
                popupTextColor={"black"}
              ...
              <Popupbox
                buttonText={
                  this.state.whatvalue
                    ? this.state.whatvalue.venue_spec_name
                    : "WHAT"
                }
                sendPopupData={data => this.receivedodrop(data, "whatvalue")}
                dropdown={this.state.whatDropdown}
                buttonColor={"#fff"}
                buttonTextColor={"#000"}
                popupColor={"white"}
                popupTextColor={"black"}
              ...
              <Popupbox
                buttonText={
                  this.state.wherevalue
                    ? this.state.wherevalue.trn_venue_name
                    : "WHERE"
                }
                width={200}
                dropdown={this.state.whereDropdown}
                buttonColor={"#fff"}
                sendPopupData={data => this.receivedodrop(data, "wherevalue")}
                buttonTextColor={"#000"}
                popupColor={"white"}
                popupTextColor={"black"}
              />
          ...
          {window.localStorage["LoginData"] && (
            <div class="logoutBtn">
              {/* <button
                type="button"
                class="venuedrop btn btn-secondary"
                onClick={() => this.logout()}
              >
                Logout
              </button> */}
      ...
      { console.log("oval arr",this.state.OvalButtonArray)}
     
        {/* {this.state.OvalButtonArray.map((item)=>
          <Subheader bgcolor={"#0f3eb0"}>
              {console.log("oval array",item)}
            <div className="home_droddownflex">
                <OvalButton count={item.count} />
        ...
        <NearByPlayGround
          addvenueProps={() =>
            this.props.addvenueProps && this.props.addvenueProps("moredetails")
          }
        />
