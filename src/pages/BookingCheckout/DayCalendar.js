...
this.props.closeHourModal&&this.props.closeHourModal();
    ...
    shouldDisableDates=(date,min,max)=>{
  	...
  	if(this.props.blockedslots){
  		...
  		if(filterRecords.length>0 && filterRecords[0].booking_slot_type!=''){
  			return true;
  		}
...
if(!this.state.fromDate || !this.state.endDate){
  ...
  body:JSON.stringify({bookingFrom:dateFormat(this.state.fromDate,'yyyy-mm-dd'),bookingTo:dateFormat(this.state.endDate,'yyyy-mm-dd'),venue_id:this.props.venudetails.venue_id,type:this.props.venudetails.availability.length>0?this.props.venudetails.availability[0].trn_availability_type:''}),
...
handleChange = (event, newValue) => {
  ...
  compnentWillMount(){
	...
	render() {
		...
		const nextmonth=moment(this.props.minDate).add(1,'M').format('YYYY-MM-DD')
...
<div className={`HourlyAvailFlexdiv ${getdiff>0?'':'fillwidth'}`}>
<div className={`FirstDiv firstmonth ${getdiff>0?'disableMonthfirst':'disableSecondCalendar'}`}>
    <MuiPickersUtilsProvider utils={DateFnsUtils}>
        ...
        <div className={`SecondDiv SecondCalendar ${getdiff>0?'disableMonth':'disableSecondCalendar'}`}>
      		    <MuiPickersUtilsProvider utils={DateFnsUtils}>
			   	<DatePicker
				   	disablePast={true}
					orientation="landscape"
			        ...
			        value={this.state.toValue?this.state.toValue:new Date(nextmonth)}
			        renderDay={(day, selectedDate, isInCurrentMonth, dayComponent) => {
